...
public class MenuLoader {
     ...
     * @param senders 接受反馈
     */
    public static void loadMenus(List<Menu> menus, CommandSender... senders) {
        // 清空
        menus.clear();
        // 关闭正在查看被查看的菜单
    ...
    public static List<String> loadMenu(File file, boolean replaceLoad) {
        ...
        if (file.isDirectory()) {
            ...
            String title = MenurSettings.MENU_TITLE.getFromMap(sets);
            InventoryType type = Arrays.stream(InventoryType.values()).filter(t -> t.name().equalsIgnoreCase(MenurSettings.MENU_TYPE.getFromMap(sets))).findFirst().orElse(null);
            List<String> shape = fixShape(MenurSettings.MENU_SHAPE.getFromMap(sets));
            List<String> openCmds = MenurSettings.MENU_OPEN_COMAMNDS.getFromMap(sets);
            HashMap<Button, List<Integer>> buttons = new HashMap<>();
            List<AbstractAction> openActions = TrAction.readActions(MenurSettings.MENU_OPEN_ACTIONS.getFromMap(sets, new ArrayList()));
            List<AbstractAction> closeActions = TrAction.readActions(Maps.getSimilarOrDefault(sets, MenurSettings.MENU_CLOSE_ACTIONS.getName(), new ArrayList<>()));
            String openRequirement = Maps.getSimilarOrDefault(sets, MenurSettings.MENU_OPEN_REQUIREMENT.getName(), null);
            List<AbstractAction> openDenyActions = TrAction.readActions(Maps.getSimilarOrDefault(sets, MenurSettings.MENU_OPEN_DENY_ACTIONS.getName(), new ArrayList<>()));
            String closeRequirement = Maps.getSimilarOrDefault(sets, MenurSettings.MENU_CLOSE_REQUIREMENT.getName(), null);
            List<AbstractAction> closeDenyActions = TrAction.readActions(Maps.getSimilarOrDefault(sets, MenurSettings.MENU_CLOSE_DENY_ACTIONS.getName(), new ArrayList<>()));
            ...
            if (errors.size() <= 0) {
                ...
                if (!replaceLoad && TrMenu.getSettings().getBoolean("OPTIONS.MENU-FILE-LISTENER.ENABLE", true)) {
                    ...
                    TConfigWatcher.getInst().addSimpleListener(file, () -> {
                        if (file.exists() && TrMenuAPI.getMenu(name) != null) {
                            Menu current = TrMenuAPI.getMenu(name);
                            if (loadMenu(file, true).size() <= 0) {
                                ...
                                Bukkit.getOnlinePlayers().stream().filter(p -> {
                                    ...
                                    return (holder instanceof MenuHolder && ((MenuHolder) holder).getMenu() == current);
                ...
                TrMenu.getMenus().add(new Menu(name, title, type, shape.size(), buttons, openRequirement, openDenyActions, closeRequirement, closeDenyActions, openCmds, openActions, closeActions, lockPlayerInv, transferArgs, forceTransferArgsAmount, bindItemLore, depends));
            }
        }
        return errors;
    }
