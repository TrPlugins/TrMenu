...
public class ListenerShortcutRightClick implements Listener {
    ...
    public void rightClick(PlayerInteractEntityEvent e) {
        ...
        if (e.getRightClicked() instanceof Player && Bukkit.getPlayer(e.getRightClicked().getUniqueId()) != null && Bukkit.getPlayer(e.getRightClicked().getUniqueId()).isOnline()) {
            e.setCancelled(TrMenuAPI.open(p, (p.isSneaking() ? TrMenu.getSettings().getString("SHORTCUT-OPEN.SNEAKING-RIGHT-CLICK-PLAYER", null) : TrMenu.getSettings().getString("SHORTCUT-OPEN.RIGHT-CLICK-PLAYER", null)), e.getRightClicked().getName()));
        }
    }
